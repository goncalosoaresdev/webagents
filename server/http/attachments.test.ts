...
void test('authenticated uploads resume, finalize, bind atomically and survive restart', async (t) => {
  ...
  const download = await fetch(`${address}/api/v1/attachments/${id}/content`, {
    headers,
  });
  ...
  await assert.rejects(attachments.prepare(result.turn.id), /content changed/);
});
